...
sampels=5008 #5008
folder_name = "/cluster/scratch/harno" + "/MODEL_SELECTION_CNO_helmholtz_pad_zero"+str(sampels)
cluster = "true"
...
for setup in settings:
    ...
    if sys.platform == "linux" or sys.platform == "linux2" or sys.platform == "darwin":
        if cluster == "true":
            if sbatch:
                string_to_exec = "sbatch --output=Out/CNO%j.out --time=5:30:00 -n 16 -G 1 --mem-per-cpu=512 --wrap=\"python3 TrainCNO.py"
            else:
