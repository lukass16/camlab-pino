...
folder_name = "/cluster/scratch/harno/MODEL_SELECTION_PINN"
cluster = "true"
Path_to_Pretrained_Network='/cluster/home/harno/Code/PrivateConvolutionalNeuralOperator/Physic_CNO/scratch/Best_Model_FNO/Best_FNO_1024'
...
for setup in settings:
    ...
    if sys.platform == "linux" or sys.platform == "linux2" or sys.platform == "darwin":
        if cluster == "true":
            if sbatch:
                string_to_exec = "sbatch  --output=Out/PINN%j.out --time=3:00:00 -n 16 -G 1 --mem-per-cpu=256 --wrap=\"python3 TrainPINNs.py"
            else:
                string_to_exec = "bsub -W 16:00 -n 8 -R \'rusage[mem=2048]\' -R \'rusage[ngpus_excl_p=1]\' python3 TrainPINNs.py"
        else:
            string_to_exec = "python3 TrainPINNs.py "
