...
folder_name = "/cluster/scratch/harno" + f"/MODEL_SELECTION_CNO_and_PhysicCNO_{which_example}"
cluster = "true"
...
for setup in settings:
    ...
    if sys.platform == "linux" or sys.platform == "linux2" or sys.platform == "darwin":
        if cluster == "true":
            if sbatch:
                string_to_exec = "sbatch --output=Out/PCNOandCNO%j.out --time=4:00:00 -n 16 -G 1 --mem-per-cpu=512 --wrap=\"python3 TrainCNOandPhysicCNO_Module.py"
            else:
                string_to_exec = "bsub -W 16:00 -n 32 -R \'rusage[mem=2048]\' -R \'rusage[ngpus_excl_p=1]\' python3 TrainCNOandPhysicCNO_Module.py"
        else:
            string_to_exec = "python3 TrainCNOandPhysicCNO_Module.py"
