...
from helper_functions.Preconditioning import Unnormalize_for_testing, Precondition_output, Create_P
import h5py
...
if len(sys.argv) == 2:
 ...
 #model_architecture_ is only relevant if Path to pretrained model==None!!
    ...
    folder = "/cluster/scratch/harno/TrainedModels/"+"Physic_informed_CNO_Preconditioning"+which_example
...
if preconditioning:
    assert (p==2)
    assert (InfoPretrainedNetwork["Path to pretrained model"]==None)
...
loss_pde=Loss_PDE(which_example=which_example,Normalization_values=Normalization_values,p=p,\
                  Boundary_decay=Boundary_decay,pad_factor=pad_factor,in_size=model_architecture_["in_size"],preconditioning=preconditioning,device=device)
Operator_loss=Loss_OP(p=p,in_size=model_architecture_["in_size"],pad_factor=pad_factor)
...
for epoch in range(epochs):
    with tqdm(unit="batch", disable=False) as tepoch:
        ...
        train_mse, train_op, train_pde = 0.0, 0.0, 0.0
        loss_OP=0
        ...
        for step, (input_batch,_) in enumerate(train_loader):
            ...
            loss_PDE= loss_pde(input=input_batch,output=output_pred_batch)
            ...
            losses['loss_OP'][-1]+=loss_OP          #values for plot
            loss_total.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            for step, (input_batch, output_batch) in enumerate(val_loader):
                ...
                output_pred_batch = model(input_batch)
                if preconditioning:
                       input_batch,output_batch,output_pred_batch=unnormalize(input_batch,output_batch,output_pred_batch)
