...
if len(sys.argv) == 2:
 ...
 #model_architecture_ is only relevant if Path to pretrained model==None!!
    ...
    folder = "/cluster/scratch/harno/TrainedModels/"+"Physic_informed_CNO"+which_example
...
loss_pde=Loss_PDE(which_example=which_example,Normalization_values=Normalization_values,p=p,\
                  Boundary_decay=Boundary_decay,pad_factor=pad_factor,in_size=model_architecture_["in_size"],D=2)
...
for epoch in range(epochs):
    with tqdm(unit="batch", disable=False) as tepoch:
        ...
        for step, (input_batch,_) in enumerate(train_loader):
            ...
            #assert (InfoPretrainedNetwork["Path to pretrained model"] is not None)
            #with torch.no_grad():
            ...
            #loss_PDE= loss_pde(input=input_batch,output=output_pred_batch,output_fix=output_fix)
            if  InfoPretrainedNetwork["Path to pretrained model"] is not None:
                 with torch.no_grad():
                     output_fix=model_fix(input_batch)
