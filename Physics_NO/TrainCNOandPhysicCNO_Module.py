...
from loss_functions.Relative_loss import Relative_error_training, Relative_loss
if len(sys.argv) == 2:
 ...
 #model_architecture_ is only relevant if Path to pretrained model==None!!
    ...
    folder = "/cluster/scratch/harno/TrainedModels/"+"CNO_and_PhyiscCNO"+which_example
...
loss_pde=Loss_PDE(which_example=which_example,Normalization_values=Normalization_values,p=p,\
                  Boundary_decay=Boundary_decay,pad_factor=pad_factor,in_size=model_architecture_["in_size"],D=1)
loss_train=Relative_error_training(p=p,pad_factor=pad_factor,input_size=model_architecture_["in_size"],device=device)
...
print('Different Lossssssssssssssssssssss')
...
for epoch in range(epochs):
    with tqdm(unit="batch", disable=False) as tepoch:
        ...
        for step, (input_batch, output_batch) in enumerate(train_loader):
            ...
            #loss_PDE= loss_pde(input=input_batch,output=output_pred_batch,output_fix=output_batch)
            loss_PDE= loss_pde(input=input_batch,output=output_pred_batch)

            loss_data=loss_train(output_pred=output_pred_batch,output=output_batch)
