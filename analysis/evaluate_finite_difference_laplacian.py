#!/usr/bin/env python3
"""
Evaluate the validity of the finite difference method for the Poisson equation.

This script loads the training set and for each input-label pair:
1. Computes the finite difference Laplacian of the label
...
from Physics_NO.loss_functions.ModulePDELoss import Unnormalize
...
def main():
    ...
    # Initialize finite difference Laplacian
    fd_laplace = FDLaplace(s=in_size, D=1.0, type=LAPLACIAN_TYPE)
    ...
    for input_batch, label_batch in train_loader:
        ...
        for i in range(input_batch.shape[0]):
            ...
            # Compute finite difference Laplacian of the label
            fd_laplacian, cut_size = fd_laplace(label_sample)
            
            # Take negative of Laplacian
            neg_fd_laplacian = -fd_laplacian
            
            # Crop input to match the Laplacian output size (due to boundary effects)
            input_cropped = input_sample.squeeze(0)[cut_size:-cut_size, cut_size:-cut_size]
            neg_fd_laplacian_squeezed = neg_fd_laplacian.squeeze(0)
            
            # Convert to numpy for calculations
            input_np = input_cropped.cpu().numpy()
            neg_laplacian_np = neg_fd_laplacian_squeezed.cpu().numpy()
    ...
    with open(output_filename, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("FINITE DIFFERENCE LAPLACIAN EVALUATION RESULTS\n")
        f.write("=" * 80 + "\n\n")
        
        f.write(f"Timestamp: {results['timestamp']}\n")
        f.write(f"Laplacian Type: {LAPLACIAN_TYPE}\n")
        ...
        f.write("the finite difference method and the expected physics.\n\n")
