...
The repository is organised into a small number of **top-level scripts** that you run and a larger set of **library modules** that those scripts import.  The diagram below shows the execution path for the most common workflow (training a CNO); afterwards each folder/file is explained in a single sentence.

```mermaid
graph TD;
    subgraph Training-time
        A[TrainCNO.py] --> B(Problems/CNOBenchmarks.py);
        B --> C(CNOModule.py);
        C --> D(training/filtered_networks.py);
    end
    subgraph Inference-time
        A2[Test* or downstream script] --> C;
    end
```

**Folder & file index**

• `TrainCNO.py`, `TrainFNO.py`, `TrainPINO_*.py` – Entry-points that parse hyper-parameters, build the chosen dataset via *Problems/*, construct the network, then launch the training loop.  
• `Problems/` – Pure **data-layer**.  Each `*Benchmarks.py` file defines:  
  - PyTorch `Dataset`s that load and normalise HDF5/NPY data.  
  - A wrapper class (e.g. `Helmholtz`, `ShearLayer`) which internally instantiates the Dataset + DataLoader **and** builds the associated model.  
• `CNOModule.py` – Implementation of the Coupled Neural Operator architecture (encoder-decoder with residual & invariant blocks).  
• `training/filtered_networks.py` – Anti-aliased convolutions and filtered Leaky-ReLU layers taken from StyleGAN-3; imported by `CNOModule.py`.  
• `training/FourierFeatures.py` – Sin/Cos positional encodings optionally concatenated to the network input.  
• `Physic_CNO/` – Physics-informed extensions (losses & trainers that add PDE residuals, boundary losses, pre-conditioning).  
• `ModelSelection*.py` – Random / grid search utilities that try many sets of JSON hyper-parameters and keep the best checkpoint.  
• `_OtherModels/` – ResNet, UNet, DeepONet baselines + their own training scripts.  **Not imported** unless you move them to the root or modify PYTHONPATH.  
• `torch_utils/`, `dnnlib/` – Low-level CUDA kernels and helpers required by the anti-aliasing layers.  Compiled on-the-fly the first time you import.  
• `download_data.py` – Convenience script that downloads all benchmark datasets (~2.8 GB) from Zenodo and places them in the expected `data/` folder.  
• Miscellaneous plotting & error-analysis scripts live in `Error_Distribution*.py`, `Physic_CNO/Additional/`, etc.  They do **not** affect training.
